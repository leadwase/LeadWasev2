...
const ADMIN_EMAIL    = 'contact@leadwase.com';
const SENDER         = { name: 'LeadWase', email: 'contact@leadwase.com' };
...
async function sendMail({ to, subject, html, attachments = [] }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data?.message || `Brevo a refusé l'envoi (HTTP ${res.status})`);
  }
    ...
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        ...
        <p style="font-size:12px;color:#888">
          Pour toute question : <a href="mailto:contact@leadwase.com">contact@leadwase.com</a>
        </p>
      </div>
    `,
  });
  ...
  return sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        ...
        <p style="font-size:12px;color:#888">
          Besoin d'aide ? <a href="mailto:contact@leadwase.com">contact@leadwase.com</a>
        </p>
      </div>
    `,
  });
  ...
  return sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        ...
        <hr style="border:none;border-top:1px solid #eee;margin:20px 0">
        <p style="font-size:12px;color:#888">
          Support : <a href="mailto:contact@leadwase.com">contact@leadwase.com</a>
        </p>
      </div>
    `,
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// 9. Rappel d'expiration d'abonnement (J-3) — tâche cron
// ─────────────────────────────────────────────────────────────────────────────
export async function notifySubscriptionExpiring({ email, firstName, plan, expiryDate }) {
  return sendMail({
    to: email,
    subject: `⏰ Votre abonnement LeadWase ${plan.toUpperCase()} expire bientôt`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        <h2 style="color:#d97706">Votre abonnement expire dans 3 jours</h2>
        <p>Bonjour ${firstName || ''},</p>
        <p>Votre abonnement <strong>${plan.toUpperCase()}</strong> arrive à échéance le
           <strong>${expiryDate}</strong>. Renouvelez-le dès maintenant pour continuer à profiter
           de toutes les fonctionnalités de votre profil LeadWase.</p>
        <a href="https://leadwase.com/dashboard.html"
           style="display:inline-block;margin-top:16px;padding:12px 24px;background:#16a34a;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold">
...
// ─────────────────────────────────────────────────────────────────────────────
// 10. Abonnement expiré → rétrogradé en Free — tâche cron
// ─────────────────────────────────────────────────────────────────────────────
export async function notifySubscriptionExpired({ email, firstName, plan }) {
  return sendMail({
    to: email,
    subject: `Votre abonnement LeadWase ${plan.toUpperCase()} a expiré`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        <h2 style="color:#c0392b">Abonnement expiré</h2>
        <p>Bonjour ${firstName || ''},</p>
        <p>Votre abonnement <strong>${plan.toUpperCase()}</strong> a expiré et votre profil est
           repassé en offre Free. Réabonnez-vous à tout moment pour retrouver toutes vos
           fonctionnalités (analytics, avis Google, formulaire de contact, etc.).</p>
        <a href="https://leadwase.com/offres.html"
           style="display:inline-block;margin-top:16px;padding:12px 24px;background:#16a34a;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold">
          Voir les offres
        </a>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// 11. Provisioning B2B — identifiants du chef d'entreprise + accès à son équipe
// ─────────────────────────────────────────────────────────────────────────────
export async function notifyB2BTeamProvisioned({ firstName, email, leadwaseId, password, teamSize }) {
  return sendMail({
    to: email,
    subject: `🎉 Votre espace entreprise LeadWase est prêt — ${leadwaseId}`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:auto">
        <h2 style="color:#16a34a">Votre commande B2B a été validée !</h2>
        <p>Bonjour <strong>${firstName}</strong>,</p>
        <p>Votre carte LeadWase et l'espace de gestion de votre équipe (${teamSize} carte${teamSize > 1 ? 's' : ''})
           sont prêts. Voici vos identifiants personnels :</p>
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:20px;margin:20px 0">
          <p style="margin:0 0 8px"><strong>Identifiant :</strong> ${leadwaseId}</p>
          <p style="margin:0"><strong>Mot de passe :</strong> ${password}</p>
        </div>
        <p>Depuis votre tableau de bord, rendez-vous sur <strong>"Mon équipe"</strong> pour retrouver
           les identifiants de chacune des cartes de vos employés et les leur distribuer.</p>
        <a href="https://leadwase.com/connexion.html"
           style="display:inline-block;margin-top:12px;padding:12px 24px;background:#16a34a;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold">
