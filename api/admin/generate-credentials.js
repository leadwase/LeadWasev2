...
export default async function handler(req, res) {
  ...
  try {
    ...
    await db.collection('profiles').doc(lwId).set({ uid: d.uid, leadwaseId: lwId, firstName: d.firstName, lastName: d.lastName, jobTitle: d.jobTitle, company: d.company, phone: d.phone, email: d.email, plan: 'free', createdAt: new Date() });
    await db.collection('credentials').doc(lwId).set({ uid: d.uid, leadwaseId: lwId, passwordHash: pwd, createdAt: new Date() });
